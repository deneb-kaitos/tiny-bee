...
const localCoverageDir = join(currentDir, 'coverage');
...
const run = async () => {
  ...
  await execa({
    ...
    shell: true,
      ...
      --reporter=v8
    node
      --env-file="${localSpecsDir}/.env"
      --experimental-test-coverage
      --test-reporter=lcov
      --test-reporter-destination="${localCoverageDir}/lcov.info"
      --test "${localSpecsDir}/*.specs.mjs"`;
};
